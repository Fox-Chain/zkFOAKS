...
pub unsafe fn hash_single_field_element(x: FieldElement) -> HashDigest {
    ...
    copy_nonoverlapping(std::ptr::addr_of!(x) as *const i128, std::ptr::addr_of_mut!(data[0].h0), size_of_val(&x));
...
pub unsafe fn hash_double_field_element_merkle_damgard(
    ...
    data[0] = prev_hash;
    let mut element = [x, y];
    copy_nonoverlapping(std::ptr::addr_of!(element) as *const HashDigest, std::ptr::addr_of_mut!(data[1]), size_of_val(&data[1]));
